...
model_fit2spat_new <- inla(
  formula = cases_number ~ 1 + pc1 + pc2 + total_millones + total_dist_mat +
    f(id.sp, model = "bym2", graph = w.loreto, group = year_re,
      control.group = list(model = "ar1")),
...
model_metrics <- tibble(
  ...
  DIC     = c(model_fit10spat$dic$dic, model_fit8spat$dic$dic, 
              model_fit5spat$dic$dic, model_fit2spat$dic$dic,model_fit2spat_new$dic$dic),
  pD      = c(model_fit10spat$dic$p.eff, model_fit8spat$dic$p.eff, 
              model_fit5spat$dic$p.eff, model_fit2spat$dic$p.eff,model_fit2spat_new$dic$p.eff),
  WAIC    = c(model_fit10spat$waic$waic, model_fit8spat$waic$waic, 
              model_fit5spat$waic$waic, model_fit2spat$waic$waic,model_fit2spat_new$waic$waic),
  pWAIC   = c(model_fit10spat$waic$p.eff, model_fit8spat$waic$p.eff, 
              model_fit5spat$waic$p.eff, model_fit2spat$waic$p.eff,model_fit2spat_new$waic$p.eff),
  logML   = c(model_fit10spat$mlik[1,1], model_fit8spat$mlik[1,1], 
              model_fit5spat$mlik[1,1], model_fit2spat$mlik[1,1],model_fit2spat_new$mlik[1,1])
)
```
...
model_metrics %>%
  ...
  tab_style(
    ...
    locations = cells_body(
      rows = Modelo %in% c("Modelo 2","Modelo 5")
    )
  )
```
...
data.cpo %>% 
  ...
  tab_style(
    ...
    locations = cells_body(
      rows = model %in% c("modelo5","modelo10")
    )
  )
