...
ggsave("./graph1.png", dpi = 300, bg = "white", height = 12, width = 10)
...
#   unnest(edu_prov) %>% 
#   ungroup() %>%
#   select(year,ubigeo_prov,hv109_recat,se) %>% 
#   mutate(
...
#     check = ifelse(hv109_recat > lower_loreto & hv109_recat<upper_loreto, "cumple", "no cumple"),
#     
#     diff_percent = ifelse(mean_loreto-hv109_recat > 0.8,"NP","P"),
#     
#     se_modelo = (upper_loreto - lower_loreto) / (2 * 1.96),
#     z = (hv109_recat - mean_loreto) / sqrt(se^2 + se_modelo^2),
#     p_value = 2 * pnorm(-abs(z)),
...
#   
#   
#   geom_point(aes(x = modelo, y = hv109_recat, shape = diff_percent), col = 'black', show.legend = F) +
#   
#   scale_shape_manual(
#     values = c(
#       "NP" = 23, 
...
#   unnest(wi_prov) %>% 
#   ungroup() %>%
#   select(year,ubigeo_prov,hv270_recat,se) %>% 
#   mutate(
#     ubigeo_prov = as.character(ubigeo_prov)
#   ) %>% 
#   
...
#   coord_flip()+
# 
#   facet_grid(ubigeo_prov~year, switch = "y", scales = "free") +
#   
#   theme_minimal()
# 
# 
