...
modelo_fh2_spatial_new <- inla(
  ...
  formula =  hv109_recat_cor_logit ~ 1 + pc1 + pc2 +  nls_year_norm +
    f(year, model = "ar1") +
    f(id.sp, model = "bym", graph = w.loreto),
...
distritos_sf_predict<-
  ...
  mutate(
    ...
    fit2_new = plogis(modelo_fh2_spatial_new$summary.fitted.values$mean),
...
tbl.yrd.full %>% 
  ...
  tab_style(
    ...
    locations = cells_body(
      rows = modelo %in% c("pc2", "pc2sp","pc2new")
    )
  )
...
distritos_sf_predict %>% 
  select(year,ubigeo,se_hv109_recat_cor,real_hv109_recat,fit8:fit2_new) %>% 
  mutate(
